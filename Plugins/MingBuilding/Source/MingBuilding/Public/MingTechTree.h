...
// Ê≥®Ê?ÔºöTArray ?êÂì°??UPROPERTY ?ÄË¶ÅÈ??ãÂ∑≤ÂÆåÊï¥ÂÆöÁæ©
USTRUCT(BlueprintType)
struct FMingBuildingUpgradeList
{
    GENERATED_BODY()

    // ??UPROPERTYÔºåÂ???FMingBuildingUpgrade ?®Ê≠§ÂæåÂ?Áæ?    TArray<FMingBuildingUpgrade> Upgrades;
};
...
/**
 * Âª∫Á??áÁ??∏Ê?ÁµêÊ? | Building Upgrade Data Structure
 * ÂÆöÁæ©Âª∫Á??áÁ??ÑÂ±¨?ßÂ??êÊú¨ | Define building upgrade properties and costs
 */
USTRUCT(BlueprintType)
struct FMingBuildingUpgrade
{
    ...
    /** ?áÁ??ÇÈ?(Áß? | Upgrade Time (seconds) */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    float UpgradeTime;

    /** ?ÄÂ§ßÂ?Á¥öÁ?Á¥?| Max Upgrade Level */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    int32 MaxUpgradeLevel;

    /** ?∂Â?Á≠âÁ? | Current Level */
    UPROPERTY(BlueprintReadOnly)
    int32 CurrentLevel;

    /** ÊØèÁ??üÂëΩ?ºÂÄçÁ? | Health Multiplier per level (1.2 = +20%) */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    float HealthMultiplier;

    /** ÊØèÁ??üÁî¢?àÁ??çÁ? | Production Multiplier per level */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    float ProductionMultiplier;

    /** ÊØèÁ??≤Á¶¶?çÁ? | Defense Multiplier per level */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
...
public:
    ...
    /** Ë®≠ÁΩÆÈªòË?Âª∫Á??áÁ??çÁΩÆ | Setup Default Building Upgrades */
    UFUNCTION(BlueprintCallable, Category = "Upgrades")
