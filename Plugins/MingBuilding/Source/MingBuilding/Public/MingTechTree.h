...
// 注�?：TArray ?�員XUPROPERTY ?�要�X�已完整定義
USTRUCT(BlueprintType)
struct FMingBuildingUpgradeList
{
    GENERATED_BODY()

    // XUPROPERTY，�XFMingBuildingUpgrade ?�此後�?�?    TArray<FMingBuildingUpgrade> Upgrades;
};
...
/**
 * 建�X��X��?結�? | Building Upgrade Data Structure
 * 定義建�X��X�屬?��X�本 | Define building upgrade properties and costs
 */
USTRUCT(BlueprintType)
struct FMingBuildingUpgrade
{
    ...
    /** 每�X�命?�倍�? | Health Multiplier per level (1.2 = +20%) */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    float HealthMultiplier;

    /** 每�X�產?��X��? | Production Multiplier per level */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
    float ProductionMultiplier;

    /** 每�X�禦?��? | Defense Multiplier per level */
    UPROPERTY(EditAnywhere, BlueprintReadOnly)
...
public:
    ...
    /** 設置默�?建�X��X�置 | Setup Default Building Upgrades */
    UFUNCTION(BlueprintCallable, Category = "Upgrades")
