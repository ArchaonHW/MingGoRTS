...
protected:
    ...
    // Ê≥?ÔºöTMap ‰∏çËÉΩ??•‰???TMap ??UPROPERTY ????    // TMap<FString, TMap<FString, float>> CharacterRelationships;

    // Ê≥?ÔºöTArray ‰∏çËÉΩ??•‰???TMap ??UPROPERTY ????    // TMap<FString, TArray<FString>> CharacterEffects;

    // Ê≥?ÔºöTArray ‰∏çËÉΩ??•‰???TMap ??UPROPERTY ????    // TMap<FString, TArray<FString>> HistoricalEvents;
