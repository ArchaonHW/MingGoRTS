...
void SeepageStage::Update(float dt, int seepageLevel) {
    ...
    if (lv != level) {
        if (lv > level) {
            ...
            scrimReduced = reduced; // 快照曲線檔——途中切設定不跳變
        } else {
            scrimT = -1.0f; // 回落即終結進行中的過渡
        }
        level = lv;
        visual = ForLevel(lv);
    }
    ...
    if (scrimT >= 0.0f && dt > 0.0f && std::isfinite(dt)) {
        scrimT += dt;
        const ScrimCurve& c = scrimReduced ? kReduced : kNormal;
...
float SeepageStage::ScrimAlpha() const {
    ...
    const float r = c.release > 0.0f
        ? (scrimT - c.attack - c.hold) / c.release : 1.0f;
