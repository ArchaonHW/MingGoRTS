...
struct SeepageVisual {
    ...
    float shadowOffset = 0.0f; // 陰影錯位幅度（世界單位，+x 向）
    float jitterAmp = 0.0f;    // 物件微移幅度（世界單位，定域正弦擺）
    int audioLevel = 0;        // AudioCues::SetSeepageLevel 值（0-2）
    bool themeOverride = false; // L3：全主題切換旗標
};
...
private:
    ...
    int level = 0;
    bool reduced = false;
    bool scrimReduced = false; // 觸發當下的曲線檔——過渡途中切設定不跳變
