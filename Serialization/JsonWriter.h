...
// JsonParser.h 只讀不寫；戰役存檔/卡檔編輯器需要把 JsonValue 文件
// 落成單一檔案，故補一個對稱的寫出器（含字串跳脫與數字格式）。
// 原居 Campaign/JsonWriter.h——G-3 CardEditor（Gameplay 層）消費時
// 發現寫出器屬引擎級序列化基建，故下移 Serialization 讓
// PotatoEngine ← Gameplay ← Campaign 依賴方向不破。
