...
UENUM(BlueprintType)
enum class ERefactoringType : uint8
{
...
UENUM(BlueprintType)
enum class ECodeSmellType : uint8
{
    ...
    CommentSmell, UMETA(DisplayName = "Comment Smell"),
    ComplexCondition, UMETA(DisplayName = "Complex Condition")
};
...
struct FCodeMetrics
{
    ...
    UPROPERTY()
    float TechnicalDebtHours;

    UPROPERTY()
    TMap<FString, float> ComplexityByMethod;
    ...
    FCodeMetrics()
        ...
        , CyclomaticComplexity(0.0f)
        , CodeDuplication(0.0f)
        , TechnicalDebtHours(0.0f)
    {}
};
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnCodeSmellDetected, FCodeSmell, Smell};
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnRefactoringCompleted, FRefactoringOperation, Operation};
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnMetricsCalculated, FCodeMetrics, Metrics};
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnOptimizationSuggestion, FString, Suggestion};
/**
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Code Refactoring")
    bool SimplifyComplexLogic(const FString& FilePath, int32 LineNumber};
    UFUNCTION(BlueprintCallable, Category = "Code Refactoring")
...
protected:
    ...
    UPROPERTY()
    TArray<FRefactoringOperation> OperationHistory;
    ...
    float CalculateCyclomaticComplexity(const FString& MethodContent};
    int32 CountCodeLines(const FString& Content};
    int32 CountCommentLines(const FString& Content};
    void StoreOperation(const FRefactoringOperation& Operation};
    void LogRefactoringActivity(const FString& Activity};
    static UMingCodeRefactoringManager* Get(UObject* WorldContextObject};
};
