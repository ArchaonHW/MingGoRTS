...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "API Documentation")
    void ScheduleAutoGeneration(float IntervalHours);
...
protected:
    ...
    UPROPERTY()
    FTimerHandle AutoGenerationTimer;
    ...
    FString ExtractComments(const FString& FileContent, int32 LineNumber);
    FString ParseDescription(const FString& Comment);
    FString ParseDetailedDescription(const FString& Comment);
    TMap<FString, FString> ParseParameters(const FString& Comment);
    FString ParseReturnDescription(const FString& Comment);
    TArray<FString> ParseExamples(const FString& Comment);
    TArray<FString> ParseSeeAlso(const FString& Comment);
    bool IsDocumentationComplete(const FAPIDocumentation& Doc);
    void GenerateMarkdownOutput(const FString& OutputPath);
    void GenerateHTMLOutput(const FString& OutputPath);
    ...
    FString GenerateSymbolHTML(const FAPIDocumentation& Doc);
