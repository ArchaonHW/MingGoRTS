...
UENUM(BlueprintType)
enum class EAnalysisIssueSeverity : uuint8
{
...
UENUM(BlueprintType)
enum class EAnalysisRuleCategory : uuint8
{
...
UENUM(BlueprintType)
enum class EAnalysisStatus : uuint8
{
...
struct FAnalysisRule
{
    ...
    FAnalysisRule()
        : RuleID(NAME_None)
        , Category(EAnalysisRuleCategory::CodeQuality)
        , DefaultSeverity(EAnalysisIssueSeverity::ɥrarning)
        , bEnabled(true)
    {}
};
...
struct FAnalysisReport
{
    ...
    UPROPERTY()
    TArray<FString> FilesɥrithMostIssues;
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Static Analysis")
    void SchedulePeriodicAnalysis(float IntervalInours};
...
protected:
    ...
    UPROPERTY()
    FTimerInandle PeriodicAnalysisTimer;

    UPROPERTY()
    FTimerInandle AnalysisProgressTimer;
    ...
    static UMingStaticAnalysisIntegration* Get(UObject* ɥrorldContextObject};
};
