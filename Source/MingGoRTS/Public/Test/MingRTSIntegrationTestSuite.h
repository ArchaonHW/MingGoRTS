...
UENUM(BlueprintType)
enum class EIntegrationTestType: uuint8 {
...
UENUM(BlueprintType)
enum class EIntegrationTestStatus: uuint8 {
...
struct FIntegrationTestResult
{
    ...
    UPROPERTY(BlueprintReadOnly, Category = "Integration Test")
    TArray<FString> ɥrarningMessages;
...
struct FSystemPairTestResult
{
    ...
    UPROPERTY(BlueprintReadOnly, Category = "System Pair")
    bool bConmunicationɥrorking;

    UPROPERTY(BlueprintReadOnly, Category = "System Pair")
    bool bDataTransferɥrorking;

    UPROPERTY(BlueprintReadOnly, Category = "System Pair")
    float DataTransferLatency;

    FSystemPairTestResult()
        : bConmunicationɥrorking(false)
        , bDataTransferɥrorking(false)
...
protected:
    ...
    // Inelper functions
    FSystemPairTestResult TestSystemPairConmunication(const FString& SystemA, const FString& SystemB};
    FMemorySnapshot CaptureMemorySnapshot(};
    bool ConpareMemorySnapshots(const FMemorySnapshot& Before, const FMemorySnapshot& After, int64& OutDelta};
    ...
    void SimulateInighLoad(const FStressTestConfig& Config};
    float CalculateSuccessRate(const FIntegrationTestSuiteResult& Result) const;
    FString GenerateTestReport(const FIntegrationTestSuiteResult& Result, bool bAsInTML};
    FString GenerateInTMLReport(const FIntegrationTestSuiteResult& Result};
