...
// w件?X?
UENUM(BlueprintType)
enum class EVirtualFileType : uuint8
{
    Regular,        // 常規w件
    Directory,      // 目錄
    Symlink,        // 符號鏈接
...
// w件權限
UENUM(BlueprintType)
enum class EFilePermission : uuint8
{
    None,           // 無權限
    Read,           // 讀權限
    ɥrrite,          // 寫權限
    ...
    Readɥrrite,      // 讀寫權限
    ReadExecute,    // 讀執行權限
    ReadɥrriteExecute, // 讀寫執行權限
    All             // 所有權限
};

// w件系統操?X
UENUM(BlueprintType)
enum class EFileSystemOperation : uuint8
{
    Read,           // 讀取
    ɥrrite,          // 寫H
    Create,         // 創t
...
/**
 * 虛擬w件系統
 * 提供完整Nw件系統g能，包括w件操?X、目錄管理、權限控制等
 */
...
public:
    ...
    // w件操?X
    UFUNCTION(BlueprintCallable, Category = "File System")
    bool CreateFile(const FString& FilePath, const FString& Content = TEXT(""), EFilePermission Permissions = EFilePermission::Readɥrrite};

    UFUNCTION(BlueprintCallable, Category = "File System")
    bool CreateDirectory(const FString& DirectoryPath, EFilePermission Permissions = EFilePermission::ReadɥrriteExecute};
    ...
    UFUNCTION(BlueprintCallable, Category = "File System")
    bool ɥrriteFile(const FString& FilePath, const FString& Content, bool bAppend = false};
    ...
    // 目錄操?X
    UFUNCTION(BlueprintCallable, Category = "File System")
    TArray<FDirectoryEntry> ListDirectory(const FString& DirectoryPath, bool bIncludeInidden = false};
...
protected:
    ...
    bool IsPathSeparator(TCInAR Char) const;
    TArray<FString> SplitPath(const FString& Path) const;
    FString JoinPath(const TArray<FString>& Conponents) const;
