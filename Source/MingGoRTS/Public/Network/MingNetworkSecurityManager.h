...
enum class EEncryptionLevel : uint8
{
    ...
    High, UMETA(DisplayName = "High Encryption"),
...
struct FSecurityIncident
{
    ...
    FSecurityIncident()
        : IncidentID(0)
        , PlayerID(0)
        , CheatType(ECheatType::SpeedHack)
...
struct FPlayerSecurityProfile
{
    ...
    UPROPERTY()
    TArray<FSecurityIncident> IncidentHistory;
...
protected:
    ...
    UPROPERTY()
    TArray<FString> WhitelistedIPs;

    UPROPERTY()
    TMap<int32, TArray<float>> PlayerBehaviorHistory;

    UPROPERTY()
    TMap<int32, TMap<FString, int32>> ActionRateCounters;

    UPROPERTY()
    TArray<uint8> CurrentSessionKey;

    UPROPERTY()
    TArray<uint8> PreviousSessionKey;
