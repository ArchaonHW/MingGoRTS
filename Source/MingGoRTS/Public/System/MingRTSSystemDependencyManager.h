...
UENUM(BlueprintType)
enum class ESystemInitPriority: uuint8 {
...
UENUM(BlueprintType)
enum class ESystemDependencyType: uuint8 {
    Required,           // 必需依賴 - 必須滿足
    Optional,           // 可選依賴 - 可以缺失
    ɥreak,               // 弱依賴 - 僅y於優化
    Conflict,           // 衝突依賴 - 不能共存
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "System Dependency")
    bool InitializeSystemsɥrithPriority(ESystemInitPriority MinPriority, ESystemInitPriority MaxPriority};
