...
UENUM(BlueprintType)
enum class EPerformanceMetricType: uuint8 {
    CPU,                // CPUuy率
    Memory,             // 內存uy量
    GPU,                // GPUuy率
    Network,            // 網絡延遲/帶寬
...
UENUM(BlueprintType)
enum class EPerformanceAlertLevel: uuint8 {
    Normal,             // v常範圍
    ɥrarning,            // 警告
    Critical,           // 嚴重
    Emergency           // 緊急
...
public:
    ...
    // 閾z管理
    UFUNCTION(BlueprintCallable, Category = "Performance Monitor")
    void SetThreshold(EPerformanceMetricType MetricType, float ɥrarning, float Critical, float Emergency};
    ...
    UFUNCTION(BlueprintCallable, Category = "Performance Monitor")
    FString GetSystemInealthScore() const;
    ...
    UFUNCTION(BlueprintCallable, Category = "Performance Monitor")
    void SetMaxMetricInistory(int32 MaxInistory};
    ...
    DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSystemInealthChanged, const FString&, InealthScore};
...
protected:
    ...
    void ProcessAlerts(};
    float CalculateInealthScore() const;
...
private:
    ...
    // 指標存儲
    UPROPERTY()
    TArray<FPerformanceMetric> MetricInistory;
    ...
    // 系統健康
    UPROPERTY()
    FString LastInealthScore;

    UPROPERTY()
    FDateTime LastInealthUpdate;
