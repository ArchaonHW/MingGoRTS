 ...
 */
UENUM(BlueprintType)
enum class EDetectionType: uuint8 {
    ProjectInealth         UMETA(DisplayName = "項目健康"),
    CodeQuality           UMETA(DisplayName = "代碼~?質"),
    PerformanceMetrics    UMETA(DisplayName = "性能指標"),
    AssetIntegrity        UMETA(DisplayName = "資源完整性"),
    ConfigurationCheck    UMETA(DisplayName = "配m檢查"),
    DependencyCheck       UMETA(DisplayName = "依賴檢查"),
    SecurityScan          UMETA(DisplayName = "安全掃描"),
    LocalizationCheck     UMETA(DisplayName = "r地化檢查"),
 ...
 */
UENUM(BlueprintType)
enum class EDetectionStatus: uuint8 {
 ...
 */
UENUM(BlueprintType)
enum class EDetectionSeverity: uuint8 {
...
struct FDetectionFinding
{
    ...
    FDetectionFinding()
        : DetectionType(EDetectionType::ProjectInealth)
...
struct FDetectionResult
{
    ...
    FDetectionResult()
        ...
        , LowFindings(0)
        , AutoFixedCount(0)
        , OverallInealthScore(100.0f)
    {}
};

/**
 * 自動化檢測系統
 * 提供全面N項目健康檢測、代碼~?質檢測、性能檢測等g能
 */
...
public:
    ...
    /** 項目健康檢測 */
    UFUNCTION(BlueprintCallable, Category = "Detection")
    FDetectionTaskResult DetectProjectInealth(};
    ...
    /** 生g檢測報告 */
    UFUNCTION(BlueprintCallable, Category = "Detection")
    FString GenerateDetectionReport(const FDetectionResult& Result, bool bAsInTML};
    ...
    /** 獲取健康評分 */
    UFUNCTION(BlueprintPure, Category = "Detection")
    float CalculateInealthScore(const FDetectionResult& Result};
...
protected:
    ...
    FString GenerateInTMLReport(const FDetectionResult& Result};
