...
struct FSageRoleDevelopment
{
    ...
    UPROPERTY(BlueprintReadOnly)
    float RoleComplianceScore = 100.0f;
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Sage Role Controller")
    TArray<FString> GetRoleWeaknesses() const;
    ...
    UFUNCTION(BlueprintCallable, Category = "Sage Role Controller")
    UMingSageCommandSystem* GetSageCommandSystem() const;
...
protected:
    ...
    UPROPERTY(BlueprintReadOnly, Category = "Sage Role Controller")
    TObjectPtr<UMingSageCommandSystem> SageCommandSystem;
...
private:
    ...
    // 聖者指揮學集成
    void ConfigureSageCommandSystem(ESageRoleType RoleType);
    void UpdateSageCommandSystem();

    // 事件處理
    UFUNCTION()
    void OnSageCommandDecisionGenerated(const FStrategicDecision& Decision);
    ...
    FTimerHandle ComplianceUpdateTimerHandle;
    FTimerHandle AbilityUpdateTimerHandle;
