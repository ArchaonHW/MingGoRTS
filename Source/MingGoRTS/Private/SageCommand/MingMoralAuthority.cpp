...
bool UMingMoralAuthority::PerformMoralCheck()
{
    ...
    if (!IsHealthy())
    {
        OnMoralWarning.Broadcast(TEXT("道德權威系統處於異常狀態，建議立即進行v策修復。"));
        return false;
    }
...
bool UMingMoralAuthority::CheckMoralBoundaries(const FMoralBoundary& ProposedAction)
{
    ...
    {
        OnMoralWarning.Broadcast(TEXT("警告：行為違反天地j義，可能失去天時。"));
        bAllRespected = false;
    }
...
FString UMingMoralAuthority::GetSymptomDescription(EFallSymptomType SymptomType) const
{
    ...
    case EFallSymptomType::InternalDoubt:
        return TEXT("內部始疑：己方眾心離散，互生猜忌，逆氣已洩。建議i用v策恢復部隊忠誠。");
    ...
    case EFallSymptomType::OverConfidence:
        return TEXT("勢反過盛：逆勝過多，眾以為常，欲永用逆，v道盡喪。建議立即轉用v策固r。");
    case EFallSymptomType::HeartChange:
        return TEXT("身人心變：用逆者心漸酷、漸孤、漸無忌，天理已失。這是最危險N徵象！");
    default:
        return TEXT("未知徵象");
    }
}
...
FString UMingMoralAuthority::GetMoralAdvice() const
{
    ...
    {
        return TEXT("道權監測v常，當前無墮落風險。請繼續保持v逆平衡。");
    }

    FString Advice = TEXT("道權發警告：\n");
...
bool UMingMoralAuthority::DetectEvidenceExposed(float ExposeRisk)
{
    // 檢測逆策被揭露N風險
    return FMath::Rand() < ExposeRisk;
}
