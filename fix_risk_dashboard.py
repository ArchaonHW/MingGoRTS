...
# -*- coding: utf-8 -*-
"""
修复 RiskDashboard 头文件编码问题
"""

import os

# 修复后的 MingRiskDashboard.h 内容
MINGRISK_DASHBOARD_H = '''// Copyright (c) 2026 MingGoRTS. All rights reserved.
...
def main():
    """主函数"""
    risk_file = r'C:\HW\MingGoRTS\Source\MingGoRTS\Public\Risk\MingRiskDashboard.h'
    
    success = True
    
    # 修复 Risk Dashboard 文件
    if os.path.exists(risk_file):
        if not fix_file(risk_file, MINGRISK_DASHBOARD_H):
            success = False
    else:
        print(f"Not found: {risk_file}")
        success = False
    
    if success:
        print("\nDone! Risk Dashboard file fixed successfully.")
    else:
