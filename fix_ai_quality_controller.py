...
# -*- coding: utf-8 -*-
"""
修复 AIContentQualityController 头文件编码问题
"""

import os

# 修复后的 AIContentQualityController.h 内容
AI_CONTENT_QUALITY_CONTROLLER_H = '''// Copyright (c) 2026 MingGoRTS. All rights reserved.
...
def main():
    """主函数"""
    ai_file = r'C:\HW\MingGoRTS\Source\MingGoRTS\Public\AI\AIContentQualityController.h'
    
    success = True
    
    if os.path.exists(ai_file):
        if not fix_file(ai_file, AI_CONTENT_QUALITY_CONTROLLER_H):
            success = False
    else:
        print(f"Not found: {ai_file}")
        success = False
    
    if success:
        print("\nDone! AI Content Quality Controller file fixed successfully.")
    else:
