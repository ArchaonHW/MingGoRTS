# MingGoRTS UE5.7 API Learning System - Fixed PowerShell Script
...
function Set-Documentation {
    ...
    try {
        ...
        Add-APIPatterns -Processor $Processor -DocData $DocData
        Add-BestPractices -Processor $Processor -DocData $DocData
    ...
    if ($DocData.APIs) {
        foreach ($API in $DocData.APIs) {
            $Pattern = @{
                ...
                "Usage" = Get-UsageExample -APIName $API
...
function Get-UsageExample {
    ...
    # Using single quotes to avoid PowerShell parsing issues with C++ syntax
    $Examples = @{
...
function New-LearningExamples {
    ...
    if ($DocData.APIs) {
        foreach ($API in $DocData.APIs) {
            $Example = @{
                "API" = $API
                "Example" = Get-UsageExample -APIName $API
                "Explanation" = Get-APIExplanation -APIName $API
                "Section" = $DocData.Section
                "Difficulty" = "Beginner"
            }
            $Examples += $Example
...
function Update-KnowledgeBase {
    ...
    $ClassesKnowledge = Get-KnowledgeFile -FilePath $ClassesPath
    ...
    if ($DocData.APIs) {
        foreach ($API in $DocData.APIs) {
            $Concept = @{
                ...
                "Type" = "Class"
                "Section" = $DocData.Section
                "Description" = Get-APIExplanation -APIName $API
                "Usage" = Get-UsageExample -APIName $API
                "LearnedAt" = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
            }
    ...
    Set-KnowledgeFile -FilePath $ClassesPath -Knowledge $ClassesKnowledge
    
    # Update patterns knowledge
    $PatternsPath = "$($Processor.KnowledgeBasePath)\Patterns\knowledge.json"
    $PatternsKnowledge = Get-KnowledgeFile -FilePath $PatternsPath
    ...
    Set-KnowledgeFile -FilePath $PatternsPath -Knowledge $PatternsKnowledge
    
    Write-Log "知识库更新完成" "SUCCESS"
}

function Get-KnowledgeFile {
