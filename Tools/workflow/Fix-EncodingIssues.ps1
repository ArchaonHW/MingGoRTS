...
Write-Host "Starting batch encoding fix..."
Write-Host "Found $totalFiles .cpp files"
...
foreach ($file in $cppFiles) {
    $processedFiles++
    Write-Progress -Activity "Fixing encoding issues" -Status "Processing $file" -PercentComplete (($processedFiles / $totalFiles) * 100)
    ...
    try {
        ...
        if ($hasEncodingIssues) {
            ...
            "Fixed file: $($file.FullName)" | Out-File -FilePath $LogPath -Append
            Write-Host "✓ Fixed: $($file.Name)" -ForegroundColor Green
        } else {
            "No encoding issues: $($file.FullName)" | Out-File -FilePath $LogPath -Append
            Write-Host "○ Skipped: $($file.Name) (no issues)" -ForegroundColor Yellow
        }
    }
    catch {
        "Error processing file $($file.FullName): $($_.Exception.Message)" | Out-File -FilePath $LogPath -Append
        Write-Host "✗ Error: $($file.Name) - $($_.Exception.Message)" -ForegroundColor Red
    }
}

Write-Host "`nBatch fix completed!"
Write-Host "Detailed log available at: $LogPath"
