...
SLIDES_EP50 = [
    {
        ...
        "title": "零成本的注視",
        "lines": ["從實戰學 C++ 第五十集",
                  "Profiler 區段計時 · chrome trace · "
                  "PSVF 簽章存檔 · lib 前綴坑"],
        "narration": "第五十集，引擎學會看自己。"
                     "區段計時器進核心——RAII 量測、"
                     "停用近乎免費、chrome 追蹤匯出。"
                     "同批落地的還有簽章存檔——"
                     "完整性從存檔開始武裝。",
    },
    {
        ...
        "title": "量測要近乎免費",
        "sections": [
            ("做法", "POTATO_PROFILE_SCOPE 展開成 "
                    "ScopedProfileZone——Current() 為空時"
                    "ctor/dtor 各一次原子讀+分支。"),
            ("代價", "停用狀態的測量面接近零——"
                    "埋點可以留在程式碼裡，"
                    "不用 #ifdef 拔掉。"),
        ...
        "code": 'POTATO_PROFILE_SCOPE("Update");\n// Current()==nullptr 時\n// 原子讀+分支 → 近零成本\n// 埋點不用 #ifdef 拔掉',
        "narration": "第一個設計：量測要近乎免費。"
                     "作用域宏展開成 RAII 計時器——"
                     "停用時建構解構各一次原子讀加分支，"
                     "測量面接近零。"
                     "埋點可以留在程式碼裡不用拔掉——"
    ...
    },
    {
        ...
        "title": "聚合與環形雙層",
        "sections": [
            ("聚合", "StatsTable：每 zone 的 "
                    "count/total/avg/min/max——"
                    "常看的數字即時可查。"),
            ("事件環", "固定容量 ring buffer 存最近 "
                    "N 筆——DumpChromeTrace 匯出 "
                    "chrome://tracing 可開的 JSON。"),
            ("啟示", "兩種觀看方式兩種結構——"
                    "統計要聚合、追蹤要時序，"
                    "一份埋點兩種輸出。"),
        ],
        "code": 'ZoneStats{count,total,avg,min,max}\nring[65536] → chrome://tracing JSON\n// 統計看聚合，追蹤看時序',
        "narration": "第二個設計：聚合與環形雙層。"
                     "統計表給每個區段的計數總量"
                     "均值極值；事件環存最近六萬多筆，"
                     "匯出成瀏覽器追蹤工具能開的格式。"
                     "統計看聚合、追蹤看時序——"
    ...
    },
    {
        ...
        "code": '// 誠實性：單 mutex 聚合\n// 微秒級 zone 有可觀 overhead\n// 設計給每幀數十~數千次',
        "narration": "第三個設計：量測自報成本。"
                     "註解明寫單一互斥鎖聚合，"
                     "微秒級區段有可觀開銷——"
                     "設計給每幀數十到數千次。"
                     "觀察者效應寫進規格："
    ...
    },
    {
        ...
        "code": 'PSVF | data | HMAC-SHA256(32B)\n// tmp+rename 原子寫\n// 驗簽失敗→object 不動',
        "narration": "第四個設計：簽章檔的三層。"
                     "魔數檔頭、原始資料、"
                     "訊息鑑別碼尾巴——"
                     "頭驗身分、尾驗完整。"
                     "暫存加改名原子寫沿用既規矩，"
    ...
    },
    {
        ...
        "sections": [
            ("發現", "MinGW 產物叫 "
                    "libPotatoBridge.dll——"
                    "DllImport(\"PotatoBridge\") "
                    "按模組名解析，找不到。"),
            ("修法", "set_target_properties "
                    "PREFIX \"\" OUTPUT_NAME——"
                    "產物名等於匯入名。"),
            ("啟示", "檔名是 ABI 的一部分——"
                    "P/Invoke 的字串和連結器"
        ...
        "code": '// libPotatoBridge.dll ✗\n// PotatoBridge.dll     ✓\nPREFIX \"\" + OUTPUT_NAME \"PotatoBridge\"\n// DllImport 字串=模組檔名',
        "narration": "第六個設計：模組名也是契約。"
                     "MinGW 產物自動帶 lib 前綴，"
                     "受控端按模組名解析找不到——"
                     "屬性覆寫去掉前綴，"
                     "產物名等於匯入名。"
    ...
    },
    {
        ...
        "title": "config 履約第二例",
        "sections": [
            ("事實", "enableProfiling 實接——"
                    "繼 enableJobSystem 之後，"
                    "第二個躺著的設定欄長出功能。"),
            ("模式", "引擎設定檔是許諾清單——"
                    "每個欄位遲早要兌現，"
                    "回步批次一批批還。"),
        ...
        "code": 'cfg.enableProfiling → Profiler 實接\n// 繼 enableJobSystem 後\n// 第二個 config 欄位履約',
        "narration": "第七個設計：設定履約第二例。"
                     "性能剖析開關實接——"
                     "繼工作池之後第二個設定欄位"
                     "長出功能。"
                     "引擎設定是許諾清單："
    ...
    },
    {
        ...
        "title": "延期清單再進貨",
        "sections": [
            ("事實", "deferred-work.md 加三項——"
                    "D-5 審查沒修的，"
                    "連理由和證據一起進清單。"),
            ("慣例", "延期條目帶 source_spec/"
                    "summary/evidence——"
                    "延到什麼時候、為什麼延，"
                    "都留字。"),
            ("啟示", "「不修」要記得比「修了」更清楚——"
        ...
        "code": 'deferred-work.md +3\n# source_spec + summary + evidence\n# 延期=給未來的包裹',
        "narration": "第八個設計：延期清單再進貨。"
                     "審查沒修的三項連理由和證據"
                     "一起進延期檔。"
                     "延期不是遺忘是給未來的包裹——"
                     "不修要記得比修了更清楚。",
    },
    {
        "kind": "cover",
        "title": "第五十集清單",
        "lines": ["近零埋點 · 聚合+環形 · 自報成本",
                  "PSVF 三層 · 完整性≠機密 · 模組名契約",
                  "config 履約 · 延期是包裹"],
        "narration": "第五十集收在這裡——整數里程碑。"
                     "引擎現在會看自己（剖析）、"
                     "會驗自己的檔（簽章）、"
                     "會給外人用（橋接）。"
                     "五十集走過的路，"
...
SLIDES_EP51 = [
    {
        ...
        "title": "決定性的紅利",
        "lines": ["從實戰學 C++ 第五十一集",
                  "Networking 模組 · LockstepSync · "
                  "輸入延遲 tick · UdpSocket"],
        "narration": "第五十一集，引擎長出網路層。"
                     "LockstepSync 做 RTS 式決定性同步——"
                     "但它能成立，是因為前面四十七集"
                     "把確定性當基礎建設做。"
                     "這集講：決定性開始發紅利。",
    },
    ...
    },
    {
        ...
        "sections": [
            ("做法", "本地輸入排到 "
                    "currentTick+delay 生效——"
                    "用延遲換網路往返時間。"),
            ("細節", "tick<delay 視為全員空指令——"
                    "開局的延遲窗用空指令墊，"
                    "不等網路也能起跑。"),
            ("啟示", "延遲是預算不是缺陷——"
                    "把 RTT 變成排程參數，"
        ...
        "code": 'inputDelayTicks = 2;\n// 本地輸入排到 tick+2\n// tick<delay → 全員空指令\n// RTT 變成排程參數',
        "narration": "第二個設計：輸入延遲買往返。"
                     "本地輸入排到未來節拍生效，"
                     "用固定延遲換網路往返時間。"
                     "開局的延遲窗用空指令墊底，"
                     "不等網路也能起跑。"
    ...
    },
    {
        ...
        "title": "收齊才推進",
        "sections": [
            ("做法", "TryAdvance：本端收齊所有 peer "
                    "對 tick T 的指令包才准推進 T——"
                    "缺任何一包就停。"),
            ("誠實", "stall 是狀態不是錯誤——"
                    "ReadyToAdvance=false 明白告訴你"
                    "在等誰。"),
            ("啟示", "停比錯好——"
                    "決定性系統寧可卡住，"
                    "不可分叉。"),
        ],
        "code": 'while (sync.TryAdvance(tc)) sim(tc);\n// 收齊所有 peer 的 T 包\n//   才准推進 T\n// 停比錯好——卡住不可分叉',
        "narration": "第三個設計：收齊才推進。"
                     "所有對等端對某節拍的指令包到齊，"
                     "才准推進那拍——缺任何一包就停。"
                     "停滯是狀態不是錯誤，"
                     "明白告訴你在等誰。"
    ...
    },
    {
        ...
        "title": "缺口補空包",
        "sections": [
            ("做法", "SealLocalPackets 補齊 "
                    "lastSealed+1 到 currentTick+delay "
                    "的全部缺口——中間補空包。"),
            ("細節", "最後一包帶走已排入的輸入；"
                    "跳 tick 時回傳多包——"
                    "封裝是補齊不是壓縮。"),
        ...
        "code": 'SealLocalPackets():\n  補齊所有缺口（中間空包）\n  最後一包帶走已排輸入\n// 序列無洞——沒收到≠沒有',
        "narration": "第四個設計：缺口補空包。"
                     "封包時補齊上次到現在之間"
                     "所有節拍——中間用空包墊。"
                     "跳節拍時回傳多個封包。"
                     "連續性用空包保證："
    ...
    },
    {
        ...
        "code": '// 本層不含可靠性/重傳\n// UDP 掉包 → stall\n// 由上層決定重送 LastLocalPacket\n//   或斷線處理',
        "narration": "第五個設計：分層寫在文件。"
                     "類註解明寫不含可靠性和重傳——"
                     "掉包造成停滯，由上層決定"
                     "重送或斷線。"
                     "同步層管順序完整性、"
    ...
    },
    {
        ...
        "title": "socket 只做一件事",
        "sections": [
            ("做法", "UdpSocket：Bind/SendTo/"
                    "RecvFrom 非阻塞——回傳 0 無資料、"
                    "-1 錯誤、正數位元組。"),
            ("細節", "port=0 系統配 ephemeral；"
                    "Close 冪等；RAII 關描述符——"
                    "封包層不管對端是誰。"),
            ("啟示", "傳輸層越笨越好——"
                    "收發兩個動詞，"
    ...
    },
    {
        ...
        "code": '// 多年前存的錢今天領：\n// seeded RNG + 無 RNG 戰鬥\n// + 回放雜湊鏈\n//   → lockstep 免費成立',
        "narration": "第八個設計：紅利的匯票。"
                     "種子隨機數、無亂數戰鬥、"
                     "回放雜湊鏈——全是為今天存的錢。"
                     "lockstep 把這些紀律變成多人同步，"
                     "決定性從測試需求升級成網路協定。"
                     "基礎建設的回報在後面："
                     "做對的時候不知道為了什麼，"
                     "用到的時候一切剛好。",
    },
    {
        "kind": "cover",
        "title": "第五十一集清單",
        "lines": ["指令不走狀態 · 延遲買往返 · 收齊才推進",
                  "缺口補空包 · 分層寫文件 · checksum 對帳",
                  "笨傳輸層 · 決定性的紅利"],
        "narration": "第五十一集收在這裡。網路層落地"
                     "上游——引擎有了第一個"
                     "需要「別人」的模組。"
                     "而它能成立，全靠那些"
                     "當年看起來只是潔癖的紀律："
...
SLIDES_EP52 = [
    {
        ...
        "title": "名字留在橋這邊",
        "lines": ["從實戰學 C++ 第五十二集",
                  "DrainEvents 部分寫入 · 快照無字串 · "
                  "名冊 Tag 綁 id · sprint 對帳"],
        "narration": "第五十二集，橋的兩端分工。"
                     "戰況卷軸越過邊界——但小隊的名字"
                     "沒有：快照裡只有編號，"
                     "名字留在 C# 那邊的字典裡。"
                     "這集講什麼過橋、什麼不過。",
    },
    {
        ...
        "sections": [
            ...
            ("理由", "字串是顯示詞彙不是狀態——"
                    "跨邊界傳 UTF-8 要處理長度/"
                    "編碼/生命周期，ID 一個 int 搞定。"),
            ("啟示", "「顯示名」歸顯示端——"
                    "資料模型裡不含的字，"
                    "就不用寫進線協定。"),
        ],
        "code": 'SquadNames[id] = name; // 部署時登錄\n// 快照只帶 id\n// 顯示詞彙留在受控端',
        "narration": "第一個設計：快照不含字串。"
                     "二進制快照只載編號和數值——"
                     "名稱字典由受控端在部署時自建。"
                     "字串是顯示詞彙不是狀態，"
                     "跨邊界要處理長度編碼生命期，"
    ...
    },
    {
        ...
        "title": "Drain 只移除寫出去的",
        "sections": [
            ("做法", "PB_DrainEvents 逐條試裝——"
                    "容量不夠就停，"
                    "只 pop_front 實際寫入的條目。"),
            ("細節", "dst=nullptr 是純清空語義——"
                    "呼叫端主動丟棄時不騰資料，"
                    "快路徑直接 clear。"),
            ("啟示", "drain 語義要部分可完成——"
                    "「全取或全留」之外還有第三態："
        ...
        "code": 'if (n + need > capacity) break;\n... ++consumed;\nwhile (consumed--) pop_front();\n// 取走放得下的 剩下留待下次',
        "narration": "第二個設計：排空只移除寫出去的。"
                     "逐條試裝、容量不夠就停，"
                     "只彈出實際寫入的條目——"
                     "剩餘事件保留待下次。"
                     "空指標是純清空語義，"
    ...
    },
    {
        ...
        "code": 'SetEventCallback([&](m){\n  snprintf(head,"[%5.1f]",\n           battle.GetElapsed());\n  events.push_back(head+m);\n});  // 源頭打戳',
        "narration": "第三個設計：時間戳在源頭打。"
                     "事件回呼在橋內攔截，"
                     "進環形佇列前打上戰鬥時間前綴。"
                     "受控端打戳會偏——"
                     "收到時間不是發生時間。"
    ...
    },
    {
        ...
        "title": "名冊綁 id 不綁列",
        "sections": [
            ("做法", "RosterList 的 ListBoxItem.Tag "
                    "= squadId——一次性建列，"
                    "逐幀只改 Content 文字。"),
            ("互動", "SelectionChanged 把 Tag 的 id "
                    "寫回 selected——UI 列和小隊"
                    "用 id 對應不靠索引。"),
            ("啟示", "UI 元素綁資料身分——"
                    "增刪重排時索引會漂，"
        ...
        "code": 'item.Tag = q.Id;      // 建列一次\nitem.Content = $"...{ord}"; // 逐幀改字\n// 選取回寫 selected=id\n// 綁 id 不綁索引',
        "narration": "第四個設計：名冊綁編號不綁列。"
                     "清單項目的標籤存小隊編號——"
                     "建列一次、逐幀只改文字，"
                     "選取時把編號寫回選取態。"
                     "介面元素綁資料身分："
    ...
    },
    {
        ...
        "code": 'selftest: squads==8 && moved>0\n        && events>0  // 橋有訊號\n// DLL 全綠 ≠ 橋通',
        "narration": "第五個設計：受控端也要自驗。"
                     "自檢測試跑十秒模擬，"
                     "斷言小隊數、有移動、"
                     "還有事件確實流出橋。"
                     "介面動態庫裡的測試全綠，"
    ...
    },
    {
        ...
        "title": "對帳按功能不按名",
        "sections": [
            ("做法", "sprint-status 對帳 auto-dev-queue："
                    "c-1 落在 GeneralDossier（非 "
                    "IntelLedger 名）、c-4 落在 "
                    "MythLayer favor（非 GodStance 名）。"),
            ("判準", "「按功能標 done」——"
                    "計畫的名字會漂，"
                    "落地的功能才是證據。"),
            ("啟示", "追蹤檔對帳用存在性——"
        ...
        "code": '# reconcile vs auto-dev-queue\n# c-1→GeneralDossier（非 IntelLedger）\n# c-4→MythLayer favor（非 GodStance）\n# 按功能標 done',
        "narration": "第六個設計：對帳按功能不按名。"
                     "衝刺狀態和開發佇列對帳——"
                     "情報帳落在將領檔案裡、"
                     "神態度落在滲透層的恩惠值裡，"
                     "名字全變了但功能都在。"
    ...
    },
    {
        ...
        "code": '# 無程式證據保持 backlog:\n# c-2 c-5 g-1 g-2 g-3 g-5 e-2 e-3\n# 留白=對帳真的在查證',
        "narration": "第七個設計：待辦也是誠實。"
                     "對帳註解明寫哪些項目無程式證據、"
                     "保持待辦——"
                     "全標完成的對帳是裝飾，"
                     "留白證明真的在查證。"
    ...
    },
    {
        "kind": "cover",
        "title": "第五十二集清單",
        "lines": ["快照無字串 · drain 部分完成 · 源頭打戳",
                  "綁 id 不綁列 · 消費端自驗 · 按功能對帳",
                  "留白即誠實 · 追平靠證據"],
        "narration": "第五十二集收在這裡。邊界兩邊"
                     "各守各的：原生側守權威時鐘和編號，"
                     "受控側守名字和呈現。"
                     "而狀態檔那邊，對帳終於追上程式碼——"
                     "不是靠改名字，"
...
SLIDES_EP53 = [
    ...
    },
    {
        ...
        "title": "旗標到事實",
        "sections": [
            ("缺口", "NoBattleResolver 只產出 "
                    "enemyAdvantage 旗標——"
                    "E-1 的判定是純函式庫，"
                    "沒有兌現者就是死旗標。"),
            ("兌現", "ApplyFirstWaveAdvantage："
                    "敵 +2CP +2intel、我軍士氣 "
                    "−0.10——戰前修正進 Gameplay 層。"),
            ("啟示", "產出物要配兌現者——"
                    "判定器回傳的欄位若無人消費，"
                    "功能只完成一半。"),
        ],
        "code": 'void ApplyFirstWaveAdvantage(\n    BattleController&, BattleResources&,\n    int enemyTeam, int playerTeam);\n// 旗標 → 機械事實',
        "narration": "第一個設計：旗標到事實。"
                     "無戰判定器只產出敵方優勢旗標——"
                     "純函式庫沒有兌現者就是死旗標。"
                     "首波優勢把它兌現成機械事實："
                     "敵方加籌碼加情報、我軍扣士氣。"
    ...
    },
    {
        ...
        "sections": [
            ("配方", "士氣 −0.10 但夾到 ≥0.10——"
                    "敗談軍心浮動，但不製造即潰。"),
            ("細節", "AdjustMorale 內部夾 0–1 且"
                    "歸零即 routing——所以先算"
                    "目標值再回推 delta 餵進去。"),
            ("啟示", "對夾取函式不能直接餵意圖——"
                    "餵 −0.10 會把 0.05 壓到 0 即潰；"
                    "精算是為了守住下限。"),
        ],
        "code": 'float target = max(0.10f,\n    s->GetMorale() - 0.10f);\ns->AdjustMorale(target - GetMorale());\n// 目標值回推 delta 保下限',
        "narration": "第二個設計：夾取要精算差值。"
                     "士氣減益夾在零點一以上——"
                     "敗談讓軍心浮動但不製造即潰。"
                     "調整函式內部會夾零到一、"
                     "歸零就潰逃，所以先算目標值"
    ...
    },
    {
        ...
        "title": "邊界又是 int",
        "sections": [
            ("做法", "簽名只吃 BattleController& +"
                    " BattleResources& + team 編號——"
                    "Gameplay 不 include Campaign。"),
            ("翻譯", "NoBattleVerdict 留在 Campaign 層——"
                    "呼叫端把裁定翻譯成 bool 和"
                    "隊伍編號再進來。"),
            ("啟示", "滲透層的 int 快照慣例再現——"
                    "跨層傳值語義不傳型別，"
        ...
        "code": '// Gameplay 只吃 int team\n// NoBattleVerdict 留 Campaign\n// 呼叫端翻譯裁定→參數',
        "narration": "第三個設計：邊界又是整數。"
                     "簽名只吃控制器資源和隊伍編號——"
                     "玩法層不引戰役型別，"
                     "裁定由呼叫端翻譯成參數。"
                     "滲透層的整數快照慣例再現："
    ...
    },
    {
        ...
        "code": 'LedgerChain campaignChain; // 殼層\nseed.debit=Civil; credit=Supply;\nseed.amount = lround(PopularSupport());\n// 不開帳→門檻恆0→全灰死碼',
        "narration": "第四個設計：為門檻先開帳。"
                     "門檻試算吃帳鏈淨額——"
                     "不開帳民心恆零，罷兵路徑全灰。"
                     "殼層持有跨章節帳鏈，"
                     "持久民心映射成期初分錄。"
    ...
    },
    {
        ...
        "title": "成功不產生假戰鬥",
        "sections": [
            ("做法", "Resolve Success → 戰鬥停在"
                    "部署態不走 BeginExecution——"
                    "結算走 E-4 的 peacePathZh 語域。"),
            ("對比", "跑一場零秒假戰鬥騙結算"
                    "會污染回放/戰報/統計——"
                    "不戰就是不戰。"),
            ("啟示", "路徑分叉在狀態機上游——"
        ...
        "code": 'if (success) {\n  peacePathZh = path;   // 和平語域\n  peaceSettlement = true;\n  // battle 停部署態\n} else ApplyFirstWaveAdvantage(...);',
        "narration": "第五個設計：成功不產生假戰鬥。"
                     "判定成功戰鬥停在部署態，"
                     "結算走第四集的和平語域。"
                     "跑一場零秒假戰鬥騙結算"
                     "會污染回放戰報統計。"
    ...
    },
    {
        ...
        "title": "開戰永遠合法",
        "sections": [
            ("做法", "CheckOptions 灰顯原因但"
                    "不鎖選項——未達標可強行嘗試，"
                    "Resolve 判 Failed 是設計內代價。"),
            ("立場", "無戰是選擇不是強制——"
                    "「開戰」永遠在選單裡，"
                    "失敗的代價是首波優勢不是拒絕。"),
            ("啟示", "灰顯是溝通不是閘門——"
        ...
        "code": '// available=false 列原因\n// 但選項仍可點\n// Failed = 設計內代價\n// 開戰永遠合法',
        "narration": "第六個設計：開戰永遠合法。"
                     "報表灰顯原因但不鎖選項——"
                     "未達標可以強行嘗試，"
                     "判定失敗是設計內的代價。"
                     "灰顯是溝通不是閘門："
    ...
    },
    {
        ...
        "code": 'Check(morale == 0.10f);   // 夾取\nCheck(!s->IsRouting());     // 不潰\nCheck(dead->GetMorale()==m0); // 死隊不動\nCheck(enemy.Morale == before);',
        "narration": "第八個設計：測試斷言邊界集。"
                     "標準套用、夾取下限、"
                     "零小隊、全滅小隊、敵隊不動——"
                     "每條斷言對應一個想像中的事故。"
                     "連全滅小隊都測："
...
SLIDES_EP54 = [
    {
        ...
        "title": "0.6 寫 0.6",
        "lines": ["從實戰學 C++ 第五十四集",
                  "最短 roundtrip · 鍵排序位元組確定性 · "
                  "AsInt UB 防護 · 浮點不等號"],
        "narration": "第五十四集，寫出的確定性。"
                     "存檔寫出去的數字，"
                     "讀回來要一模一樣——"
                     "而且要好看：零點六就寫零點六，"
                     "不寫十七位的尾巴。"
    ...
    },
    {
        ...
        "sections": [
            ("問題", "%g 預設六位有效——"
                    ...
                    "存檔悄悄失真。"),
            ("修法", "先改 %.17g 全精度——"
                    "17 位保證 double roundtrip 無損。"),
            ("啟示", "序列化的第一公理是無損——"
                    "好看是第二公理，"
                    "但第一公理不能退讓。"),
        ],
        "code": '// %g: 6 位 → >1e6 截斷\n// %.17g: 全精度無損\n// 但 0.6 → 0.59999999999999998',
        "narration": "第一個設計：百分比 g 六位會截斷。"
                     "預設六位有效數字，"
                     "超過一百萬的整數被截斷——"
                     "存檔悄悄失真。"
                     "先改成十七位全精度保無損，"
    ...
    },
    {
        ...
        "sections": [
            ("做法", "由 6 位起逐步升精度——"
                    "strtod 讀回原值的第一個"
                    "精度就用，最多升到 17。"),
            ("效果", "0.6 寫 0.6、整數不帶小數點，"
                    "任意 double 仍保證無損回讀。"),
            ("啟示", "驗證式寫出——"
                    "寫之前先證明讀得回來，"
                    "格式的品質由讀端裁決。"),
        ],
        "code": 'for (int prec = 6; prec <= 17; ++prec) {\n  snprintf(buf, "%.*g", prec, v);\n  if (strtod(buf,nullptr) == v) break;\n}  // 第一個讀回原值的就用',
        "narration": "第二個設計：最短往返。"
                     "從六位開始逐步升精度，"
                     "讀回原值的第一個精度就用——"
                     "零點六寫零點六，"
                     "任意雙精度仍無損。"
    ...
    },
    {
        ...
        "title": "鍵排序抗漂移",
        "sections": [
            ("問題", "unordered_map 迭代序不定——"
                    "同一份資料寫出不同位元組，"
                    "replay/golden 比對全廢。"),
            ("修法", "JsonWriter object 鍵排序輸出；"
                    "MythLayer 的 DeriveFrom/ToJson "
                    "同因排序。"),
            ("啟示", "位元組確定性是可比對的前提——"
                    "檔案要能 diff，"
                    "雜湊要穩定。"),
        ],
        "code": '// unordered_map 序不定\n// → 鍵排序後寫出\n// 插入序不同 位元組相同',
        "narration": "第三個設計：鍵排序抗漂移。"
                     "無序映射的迭代序不定——"
                     "同一份資料寫出不同位元組，"
                     "回放和金樣比對全廢。"
                     "寫出器把物件鍵排序，"
    ...
    },
    {
        ...
        "title": "AsInt 先夾取再轉型",
        "sections": [
            ("問題", "double→int 轉型越界是 UB——"
                    "NaN、1e30 進 AsInt 行為不定。"),
            ("修法", "先夾到 INT_MIN/INT_MAX 再轉型——"
                    "毒值在邊界被接住不往下游傳。"),
            ("啟示", "轉型點即防線——"
                    "D-5 審查在消費端抓過一次，"
                    "這次把防線收進讀取器本身。"),
        ],
        "code": 'int AsInt() const {\n  if (!isfinite(numberValue))\n    return 0;\n  return (int)clamp(numberValue,\n    INT_MIN, INT_MAX); // 夾完才轉\n}',
        "narration": "第四個設計：轉整數先夾取再轉型。"
                     "浮點轉整數越界是未定義行為——"
                     "非數和天文數字進來行為不定。"
                     "先夾到整數範圍再轉型，"
                     "毒值在邊界被接住。"
    ...
    },
    {
        ...
        "code": '// 亂序插入 → ToJson 位元組相同\nCheck(jsonA == jsonB, \"插入序無關\");\n// 漂移變回歸 不變量釘在 CI',
        "narration": "第五個設計：插入序無關斷言。"
                     "滲透層測試補上：不同插入順序"
                     "產生位元組相同的輸出。"
                     "會不會漂移不再是審查完就忘的問題，"
                     "斷言把不變量釘在持續整合裡。"
    ...
    },
    {
        ...
        "title": "浮點沒有等號",
        "sections": [
            ("收尾", "NoBattleAdvantageTest 的"
                    " morale == m−0.10f 改 Near——"
                    "逐位相等對浮點是僥倖。"),
            ("理由", "0.1 無法精確表示——"
                    "夾取加減後的位元組"
                    "和直寫字面量可能差一位。"),
        ...
        "code": 'Check(Near(morale, m0-0.10f));\n// Near: |a-b| < 0.001\n// 浮點==是僥倖 epsilon是斷言',
        "narration": "第六個設計：浮點沒有等號。"
                     "優勢測試的士氣等值斷言"
                     "改成近似比較——"
                     "零點一無法精確表示，"
                     "夾取運算後的位元組"
    ...
    },
    {
        ...
        "code": '// PBS1 二進制: 機器讀\n// potato.campaign JSON: 人讀\n// 0.6 寫 0.6 為了眼睛',
        "narration": "第八個設計：可讀也是契約。"
                     "零點六寫零點六不只是美觀——"
                     "存檔是人會打開看的檔案。"
                     "二進制快照不講可讀、"
                     "文字存檔講——"
    ...
    },
    {
        "kind": "cover",
        "title": "第五十四集清單",
        "lines": ["六位截斷 · 最短 roundtrip · 鍵排序",
                  "AsInt 夾取 · 插入序斷言 · 浮點無等號",
                  "端到端確定 · 為讀者寫出"],
        "narration": "第五十四集收在這裡。"
                     "序列化學會了兩件事："
                     "寫出去的每個位元組都要確定，"
...
SLIDES_EP55 = [
    {
        ...
        "title": "遊戲自己拍宣傳片",
        "lines": ["從實戰學 C++ 第五十五集",
                  "MingGoRTS.Trailer · 橋驅動真實戰鬥 · "
                  "軟體渲染逐幀 · 三段式管線移植"],
        "narration": "第五十五集，最 meta 的一集。"
                     "宣傳片的每一格戰鬥畫面"
                     "都不是錄影——是 C# 客戶端透過橋"
                     "驅動真實無頭戰鬥逐幀渲染的。"
                     "遊戲自己拍了自己的宣傳片。",
    },
    {
        ...
        "title": "畫面是真實模擬",
        "sections": [
            ("做法", "GameSession.CreateDuanqiao 開真局——"
                    "30fps 逐幀 Step + Snapshot，"
                    "畫面來自引擎不是錄影。"),
            ("代價", "沒有「拍出來剛好帥」的運氣——"
                    "霧、指令線、士氣條全是真狀態，"
                    "不好看的幀就是真實的幀。"),
        ...
        "code": 'battle ??= GameSession.CreateDuanqiao();\nvar snap = battle.Step(1.0f/30f);\nRender.Battle(snap, ...);\n// 每格都是真模擬',
        "narration": "第一個設計：畫面是真實模擬。"
                     "宣傳片的戰鬥段不是錄影——"
                     "客戶端開真局，三十幀逐幀"
                     "推進快照渲染。"
                     "霧、指令線、士氣條全是真狀態，"
    ...
    },
    {
        ...
        "title": "管線跨語言移植",
        "sections": [
            ("對齊", "三段式和 gen_cpp_tutorial.py "
                    "同一條：TTS→影格→ffmpeg concat。"),
            ("細節", "TTS 同走 PowerShell "
                    "System.Speech Hanhan——免 "
                    "NuGet 依賴，聲音和教學片同源。"),
            ("啟示", "好管線會自己繁殖——"
                    "模式證明有效後，"
                    "下一個產物直接沿用形狀。"),
        ],
        "code": '// Python: SAPI→PIL→ffmpeg\n// C#: System.Speech→DrawingVisual→ffmpeg\n// 同一條產線 換語言不換形狀',
        "narration": "第二個設計：管線跨語言移植。"
                     "三段式和教學產生器同一條——"
                     "語音合成、影格、合成。"
                     "連聲音都是同一個桌面語音，"
                     "免套件依賴。"
    ...
    },
    {
        ...
        "title": "軟體渲染無視窗",
        "sections": [
            ("做法", "DrawingVisual→RenderTargetBitmap→"
                    "PNG——全程 CPU，"
                    "不需視窗不需 GPU。"),
            ("延續", "字體沿用 F-5 bundled CJK——"
                    "找不到退回系統正黑體，"
                    "降級鏈又活了一次。"),
            ("啟示", "無頭哲學貫穿到行銷——"
                    "連宣傳片都在 CI 上能跑。"),
        ],
        "code": 'RenderTargetBitmap bmp(W,H,96,96,\n  PixelFormats.Pbgra32);\nbmp.Render(visual); enc.Save(png);\n// 無視窗無GPU 宣傳片也能CI',
        "narration": "第三個設計：軟體渲染無視窗。"
                     "繪圖視覺樹光柵化成位圖存 PNG——"
                     "全程處理器，不需視窗不需顯卡。"
                     "字體沿用打包的 CJK，"
                     "找不到退回系統字體。"
    ...
    },
    {
        ...
        "code": 'case "probe":\n  fog = Fogs.First(f => !f.Revealed);\n  c = fog.Candidates.OrderByDescending(\n      c => c.Prob).First();\n  PB_FogProbe(h, fog.EntityId, c.X, c.Y);\n// 宣傳片真的在探測',
        "narration": "第四個設計：腳本指令調度戰場。"
                     "戰鬥場景帶提示指令——"
                     "推進令前鋒貫穿缺口，"
                     "探測令找最高機率候選格下探。"
                     "探測收斂機率雲不是文案，"
    ...
    },
    {
        ...
        "code": 'double dur = Math.Max(durs[i],\n    sc.MinSeconds); // 旁白 vs 畫面下限\n// AudioPad=0.6 尾留喘息',
        "narration": "第五個設計：時長兩個下限。"
                     "每段長度取旁白時長和"
                     "最短秒數的大者——聲音說多久、"
                     "畫面至少演多久，誰長聽誰。"
                     "旁白後留零點六秒喘息。"
    ...
    },
    {
        ...
        "code": '// seg_00.mp4 ... seg_05.mp4\nffmpeg -f concat -c copy\n// 流拷貝不重壓 壞一段補一段',
        "narration": "第六個設計：分段編碼再拼接。"
                     "每場景獨立編碼成片段，"
                     "拼接用流拷貝不二次壓縮。"
                     "單段壞了只重跑一段——"
                     "產線分段的價值在局部重跑，"
    ...
    },
    {
        ...
        "code": '// 一次生成 = 全程橋走查\n// Create→Cue→Snapshot→Destroy\n// 能拍片 = API 真的可用',
        "narration": "第七個設計：宣傳片是整合測試。"
                     "一次生成走完整條橋："
                     "開局、部署、介入、探測、"
                     "逐幀快照、銷毀。"
                     "橋能拍出片，就證明消費者"
...
SLIDES_EP56 = [
    {
        ...
        "title": "粒子不畫自己",
        "lines": ["從實戰學 C++ 第五十六集",
                  "ParticleSystem 純模擬 · 流場回呼 · "
                  "容量截斷 · 同 seed 同序列"],
        "narration": "第五十六集，粒子系統的範圍誠實。"
                     "這個類叫粒子系統，"
                     "卻一行繪製碼都沒有——"
                     "它只做模擬，渲染是別人的事。"
                     "這集講「不畫」為什麼是設計。",
    },
    {
        ...
        "sections": [
            ("宣言", "標頭註解開門見山：只做"
                    "發射/積分/壽命/流場，"
                    "不含 GL——Particles() 唯讀陣列"
                    "交給呼叫端對接。"),
            ("收穫", "無頭可測是免費的——"
                    "模擬層不碰 GL，"
                    "CI 上跑完整契約測試。"),
            ("啟示", "渲染是分離的接縫——"
                    "SpriteAtlas/點精靈/instancing "
                    "由消費端選，模擬不預設。"),
        ],
        "code": '// 只做模擬 不含 GL 繪製\n// 渲染端拿 Particles() 自行對接\nconst std::vector<Particle>&\n  Particles() const;  // 唯讀出口',
        "narration": "第一個設計：模擬不含繪製。"
                     "標頭註解開門見山：只做發射積分"
                     "壽命流場，繪製由呼叫端拿唯讀陣列"
                     "自行對接。"
                     "回報是無頭可測免費——"
    ...
    },
    {
        ...
        "sections": [
            ("做法", "SetFlowField 收 "
                    "std::function(pos,time)→accel——"
                    "TurbulenceField/GustField 的 "
                    "Sample 直接包進來。"),
            ("邊界", "Rendering 不認得 Quasi——"
                    "湍流/陣風型別留在原層，"
                    "回呼是最小介面。"),
            ("啟示", "依賴用型別擦除換——"
                    "模板/回呼讓消費端組裝，"
        ...
        "code": 'void SetFlowField(std::function<\n  Vector3(const Vector3&,float)>);\n// turb.Sample(p,t) 包進來\n// Rendering 不認得 Quasi',
        "narration": "第二個設計：流場是回呼不是型別。"
                     "接環境場收的是函式——"
                     "湍流場和陣風場的取樣函式"
                     "直接包進來，渲染層不認得"
                     "上游型別。"
    ...
    },
    {
        ...
        "code": 'emitAccumulator += rate * dt;\nwhile (emitAccumulator >= 1.0f) {\n  SpawnOne(); emitAccumulator -= 1.0f;\n}  // 分數額滾到下一幀',
        "narration": "第三個設計：發射率有小數餘額。"
                     "每秒一百顆配六十幀是每幀一點六七顆——"
                     "捨去漏三分之一，進位多噴。"
                     "累加器把分數額滾到下一幀，"
                     "夠整數才生。"
    ...
    },
    {
        ...
        "code": 'ParticleSystem ps(4096);  // 上限\nBurst 超過 → 截斷\n// 寧可少噴 不可爆記憶體',
        "narration": "第四個設計：容量截斷不溢出。"
                     "粒子池建構時定上限，"
                     "爆發超過容量直接截斷。"
                     "特效過載時寧可少噴"
                     "不可爆記憶體——"
    ...
    },
    {
        ...
        "sections": [
            ("做法", "SampleCone：主方向周圍"
                    ...
                    "正交基 t/b/d。"),
            ("邊界", "方向接近垂直時換參考軸——"
                    "|d.y|>0.99 用 Right 不用 Up，"
                    "叉積退化的經典陷阱。"),
            ("啟示", "隨機方向不是隨機向量——"
                    "球面均勻分佈要從極角"
                    "開始算。"),
        ],
        "code": 'up = |d.y|<0.99 ? Up() : Right();\nt = up.Cross(d); b = d.Cross(t);\nreturn t*cosφsinθ + b*sinφsinθ + d*cosθ;\n// 正交基退化先擋',
        "narration": "第五個設計：圓錐取樣建正交基。"
                     "在主方向周圍均勻取樣——"
                     "先算極角餘弦範圍再繞方位角轉，"
                     "局部座標乘正交基。"
                     "方向接近垂直時換參考軸，"
    ...
    },
    {
        ...
        "sections": [
            ("做法", "mt19937 內建種子 "
                    "0x9E3779B9，SetSeed 公開——"
                    "決定性是 API 不是彩蛋。"),
            ("用途", "重播/測試/lockstep 都要——"
                    "特效在決定性模擬裡"
                    "不能是自由變數。"),
            ("啟示", "RNG 暴露種子是引擎慣例——"
                    "和 BattleSandbox、RL env "
                    "同一條紀律線。"),
        ],
        "code": 'std::mt19937 rng{0x9E3779B9u};\nvoid SetSeed(uint32_t s) { rng.seed(s); }\n// 決定性是 API 不是彩蛋',
        "narration": "第六個設計：種子進公開介面。"
                     "亂數產生器內建固定種子，"
                     "重設種子是公開方法——"
                     "重播、測試、鎖步都需要特效"
                     "不是自由變數。"
    ...
    },
    {
        ...
        "code": '// cone=0, v=2, dt=0.5\nCheck(|p.y - 1.0f| < 1e-4f);\n// 200顆全在30°錐內\n// 分佈契約全體斷言',
        "narration": "第七個設計：測試是積分方程式。"
                     "圓錐角零配定速，斷言位移恰等"
                     "速度乘時間；重力斷言下墜、"
                     "阻力斷言衰減。"
                     "兩百顆全體檢查都在三十度錐內——"
    ...
    },
    {
        ...
        "code": 'Quasi::TurbulenceField turb(...);\nps.SetFlowField([&](p,t){\n  return turb.Sample(p, t); });\n// 真湍流插得上才算介面對',
        "narration": "第八個設計：流場實接通配。"
                     "測試不只接假風場——"
                     "真的湍流場直接包進回呼，"
                     "三十二顆跑兩秒斷言位移非零"
                     "且全部有限。"
...
SLIDES_EP57 = [
    {
        ...
        "title": "契約寫成測試",
        "lines": ["從實戰學 C++ 第五十七集",
                  "NetworkingTest · 畸形包拒絕矩陣 · "
                  "兩端逐位一致 · localhost 整合"],
        "narration": "第五十七集，網路模組的考卷。"
                     "EP51 講過 lockstep 的設計——"
                     "這集講它的測試怎麼把契約"
                     "寫成可執行的句子："
                     "每條斷言都是一句「必須如此」。",
    },
    {
        "tag": "設計 1",
        "title": "localhost 是唯一舞台",
        "sections": [
            ("做法", "全部測試只打 127.0.0.1——"
                    "兩 socket 綁 ephemeral port "
                    "互送，真 UDP 真封包。"),
            ("收斂", "非阻塞收不到回 0 也驗——"
                    "「沒資料」是契約的一部分，"
                    "不是例外。"),
        ...
        "code": 'a.Bind(0); b.Bind(0);  // ephemeral\nSendTo("127.0.0.1", b.LocalPort());\nRecvFrom(...) == 0  // 無資料也驗',
        "narration": "第一個設計：本機迴環是唯一舞台。"
                     "全部測試只打迴環位址——"
                     "兩個通訊端綁臨時埠互送，"
                     "真協議真封包。"
                     "連收不到資料回零都驗："
    ...
    },
    {
        ...
        "code": 'Expect(!OnPeerPacket({}),"空包");\nExpect(!OnPeerPacket(bad), 壞magic);\nExpect(!OnPeerPacket(trunc),截斷);\nExpect(!OnPeerPacket(self),自封);\nExpect(!OnPeerPacket(early),過早);',
        "narration": "第二個設計：拒絕比接受更值得測。"
                     "空包、壞魔數、截斷、"
                     "冒充自己的包、太早期滴答的包——"
                     "五種壞包全斷言拒絕。"
                     "解析器的價值在邊界："
    ...
    },
    {
        ...
        "title": "記憶體直送先驗語義",
        "sections": [
            ("做法", "兩個 LockstepSync 記憶體"
                    "互餵——不經 socket，先把"
                    "「收齊才推進」的語義驗乾淨。"),
            ("斷言", "對端包未到 → !ReadyToAdvance"
                    "——stall 狀態本身是斷言對象。"),
            ("啟示", "分層測試：語義層用記憶體、"
                    "傳輸層用真 socket——"
                    "一層壞了好定位。"),
        ],
        "code": 'for (auto& p : s0.SealLocalPackets())\n  s1.OnPeerPacket(p); // 記憶體直送\nExpect(!s0.ReadyToAdvance(),\n  "缺對端包→stall"); // stall可斷言',
        "narration": "第三個設計：記憶體直送先驗語義。"
                     "兩個同步器不透過通訊端互餵——"
                     "先把收齊才推進的語義驗乾淨，"
                     "連停滯狀態本身都是斷言對象。"
                     "分層測試：語義層用記憶體、"
    ...
    },
    {
        ...
        "title": "決定性斷言逐位比",
        "sections": [
            ("做法", "tick4 雙方各自 TryAdvance——"
                    "取出的 TickCommands 逐位比對："
                    "peerId、指令內容全等。"),
            ("意義", "「兩端看到同一批指令」"
                    "是 lockstep 的全部意義——"
                    "斷言不比大概，比逐位。"),
        ...
        "code": 's0.TryAdvance(t0); s1.TryAdvance(t1);\nfor (i...) if (t0.inputs[i] !=\n    t1.inputs[i]) sameOrder=false;\n// 逐位一致 不是差不多',
        "narration": "第四個設計：決定性斷言逐位比。"
                     "同一滴答兩端各自推進，"
                     "取出的指令集逐位比對——"
                     "來源編號、內容全等。"
                     "兩端看到同一批指令"
    ...
    },
    {
        ...
        "title": "checksum 抓異議者",
        "sections": [
            ("做法", "RecordLocalChecksum 本地記帳，"
                    "peer 包進來後 ChecksumMismatches"
                    "回傳不符的 peerId 清單。"),
            ("斷言", "一致→空清單；不符→"
                    "清單精確含 peer1——"
                    "誰分歧要點名。"),
        ...
        "code": 'RecordLocalChecksum(10, 0xDEADBEEF);\nChecksumMismatches(10).empty();\n// 不符 → bad == {1}\n// 誰分歧要點名',
        "narration": "第五個設計：校驗碼抓異議者。"
                     "本地記帳、對端封包進來後"
                     "查不符清單——一致回空，"
                     "不符精確點名哪個對端"
                     "哪個滴答分歧。"
    ...
    },
    {
        ...
        "code": 'for (round<30 && advanced<5) {\n  Seal→SendTo→Recv→TryAdvance }\nExpect(advanced>=5);\nExpect(tick_a == tick_b); // 斷言果',
        "narration": "第六個設計：整合在迴環上跑真槍。"
                     "同步器乘真通訊端——每輪封口、"
                     "互送、收包、推進，跑五個滴答。"
                     "接收重試給排程餘裕，"
                     "但斷言的是結果不是過程。"
    ...
    },
    {
        ...
        "title": "stderr 會餓死管線",
        "sections": [
            ("事故", "Trailer 的 ffmpeg 進度全走"
                    " stderr——同步先讀 stdout "
                    "再讀 stderr，緩衝區滿互等死鎖。"),
            ("修法", "stderr 改 ReadToEndAsync——"
                    "兩條管線平行抽乾。"),
            ("啟示", "子行程三條管都要抽——"
                    "只讀一條等它滿，"
                    "經典死鎖第一名。"),
        ],
        "code": '// ffmpeg 進度全走 stderr\nvar errTask =\n  p.StandardError.ReadToEndAsync();\nvar outp = p.StandardOutput\n  .ReadToEnd(); // 兩管平行抽',
        "narration": "第七個設計：錯誤流會餓死管線。"
                     "宣傳片產生器卡住——編碼器進度"
                     "全走錯誤流，同步先讀輸出再讀錯誤，"
                     "緩衝區滿了互等死鎖。"
                     "修法是錯誤流改非同步讀，"
    ...
    },
    {
        ...
        "code": 'PB_SetTimeScale(h, 0.5f);\n// 慢動作=模擬參數\n// 不是後製是引擎能力',
        "narration": "第八個設計：慢動作是介面參數。"
                     "宣傳片要慢鏡頭——不調幀率，"
                     "呼叫時間縮放讓模擬自己慢下來。"
                     "接戰潰逃在鏡頭裡留得住，"
                     "鏡頭語言是模擬參數不是後製。"
...
SLIDES_EP58 = [
    {
        ...
        "title": "隱形卡的對手",
        "lines": ["從實戰學 C++ 第五十八集",
                  "CardValidator · 降級的陰影面 · "
                  "error/warn 分層 · E-3 倒戈草案"],
        "narration": "第五十八集，降級的陰影面。"
                     "載入器對壞資料靜默降級——"
                     "玩家看不到錯，只看到一張"
                     "「好像能跑」的隱形卡。"
                     "這集講驗證器如何把隱形"
    ...
    },
    {
        ...
        "title": "靜默降級的陰影",
        "sections": [
            ("問題", "未知 trigger/action 載入後"
                    "降級成 Always/HoldPosition——"
                    "卡能跑但語義全錯，"
                    "錯誤被降級藏起來。"),
            ("代價", "降級保了玩家體驗，"
                    "坑了內容作者——"
        ...
        "code": '// 未知 trigger → Always 隱形卡\n// 載入器靜默降級 不擋\n// → 需要驗證器顯式說錯',
        "narration": "第一個設計：靜默降級的陰影。"
                     "未知的觸發和動作載入後"
                     "降級成預設行為——卡能跑"
                     "但語義全錯。"
                     "降級保了玩家體驗，"
    ...
    {
        "tag": "設計 2",
        "title": "error 拒存 warn 放行",
        "sections": [
            ("分層", "error=語義必錯（壞 JSON/"
                    "缺 id/未知 trigger）；"
                    "warn=有預設可降級"
                    "（人格軸缺漏→50、越界→clamp）。"),
            ("判定", "Ok()=無 error——"
        ...
        "code": 'struct CardIssue {\n  bool error;     // 拒存級\n  string field;   // 路徑\n  string message; };\nbool Ok() // warn 不擋',
        "narration": "第二個設計：錯誤拒存警告放行。"
                     "問題分兩層——載入後語義必錯的"
                     "是錯誤，有預設值可降級的是警告。"
                     "報告的通過條件是無錯誤，"
                     "警告只記不擋。"
    ...
    },
    {
        ...
        "title": "合法值表一表兩吃",
        "sections": [
            ("做法", "TriggerNames/ActionNames/"
                    "RarityNames/UnitClassNames "
                    "公開回傳 nullptr 結尾字串表。"),
            ("消費", "編輯器 UI 拿它列下拉選單、"
                    "驗證器拿它比對合法值、"
                    "測試拿它對帳——同一張表。"),
        ...
        "code": 'static const char* const*\n  TriggerNames(); // nullptr結尾\n// UI列舉+驗證比對+測試對帳\n// 一張表三個消費者',
        "narration": "第三個設計：合法值表一表兩吃。"
                     "觸發、動作、稀有度、兵種的"
                     "合法字串表公開——編輯器拿它"
                     "列選單、驗證器拿它比對、"
                     "測試拿它對帳。"
    ...
    },
    {
        ...
        "code": 'ValidateDir(dir): 遞迴掃*.json\n// 壞檔記自己report 不中止\n// +跨卡檢查: id重複\n// 批次失敗粒度=單檔',
        "narration": "第四個設計：掃目錄不為一卡停。"
                     "遞迴掃描每張卡各得一份報告——"
                     "單卡壞掉不中止整批。"
                     "目錄級還能補跨卡檢查："
                     "編號重複只有看全部才抓得到。"
    ...
    },
    {
        ...
        "title": "情報是消耗品",
        "sections": [
            ("做法", "HearsayEntry 加 consumed 位——"
                    "顛覆成功才消耗 verified 判詞，"
                    "Gather 計數排除已消耗。"),
            ("邊界", "Failed/Backfired 不消耗——"
                    "失敗的代價是敵獲先機，"
                    "情報網還在。"),
            ("啟示", "資產消耗掛在結果上——"
                    "嘗試不收費、成交才收，"
        ...
        "code": 'ConsumeVerified(): 成功才消耗\nGather: verifiedIntel 排除 consumed\n// 失敗不燒情報 網還在\n// 消耗掛結果不掛嘗試',
        "narration": "第六個設計：情報是消耗品。"
                     "判詞條目加消耗位——"
                     "顛覆成功才燒掉已驗證的情報，"
                     "蒐集時不再計入已消耗者。"
                     "失敗不消耗：代價是敵方先機，"
    ...
    },
    {
        ...
        "code": '// 全倒戈→首tick Victory\n// 不開「無戰勝利」特例\n// 極端輸入走通用規則\n// 不戰而勝是湧現',
        "narration": "第七個設計：不戰而勝是湧現。"
                     "全數倒戈是合法輸入——"
                     "第一個滴答的勝利判定自然接住，"
                     "不開無戰勝利特例。"
                     "通用規則能自然涵蓋的極端情況，"
    ...
    },
    {
        ...
        "code": '# POTATO_TESTS 重複列名\n# whitelist: "...SimpleExample \\nNetworkingTest"\n# 字面\\n → 名字不存在 → 靜默漏同步',
        "narration": "第八個設計：清單也會寫錯。"
                     "測試名單重複列名——"
                     "區塊重放碰撞第三次出現。"
                     "同步腳本的白名單混進字面換行符，"
                     "名字不存在就靜默漏同步。"
...
SLIDES_EP59 = [
    ...
    },
    {
        ...
        "title": "翻面是隊伍的動詞",
        "sections": [
            ("做法", "Squad::Defect(newTeam)——"
                    "team 翻面 + engageTarget 清空 +"
                    " order=Hold，一件事三個動作。"),
            ("理由", "只改 team 不夠——"
                    "舊交戰目標還指著新隊友，"
                    "舊指令還是敵方下達的。"),
        ...
        "code": 'void Squad::Defect(int newTeam) {\n  team = newTeam;\n  engageTarget = nullptr;\n  order = SquadOrder::Hold;\n} // 翻面+清場',
        "narration": "第一個設計：翻面是隊伍的動詞。"
                     "叛變方法做三件事——換陣營、"
                     "清空交戰目標、指令歸待命。"
                     "只改隊伍旗不夠：舊目標還指著"
                     "新隊友，舊指令還是敵方下的。"
    ...
    },
    {
        ...
        "code": 'sort(candidates, [](a,b){\n  morale↑ → members↑ → name↑ });\n// 潰逃/全滅不列候選\n// 敘事=比較器',
        "narration": "第二個設計：士氣最弱者先倒。"
                     "候選按士氣、兵員、名稱"
                     "逐級升冪排序——三級鍵全確定性。"
                     "已潰逃和已全滅不列候選："
                     "逃跑的人不需要策反。"
    ...
    },
    {
        ...
        "code": 'fallback.AddRule({Always,\n  AttackNearest, 0, 100});\nbattle.AssignDoctrine(s, fallback);\n// 不留敵腳本 不無令待機',
        "narration": "第三個設計：倒戈隊要補新腦。"
                     "敵方教條對新陣營無意義，"
                     "而沒有教條的隊伍永遠評估不到"
                     "等於原地發呆。"
                     "補一條恆真攻擊最近的兜底——"
    ...
    },
    {
        ...
        "title": "只能在部署階段翻",
        "sections": [
            ("邊界", "呼叫端必須在 BeginExecution"
                    " 前呼叫——AssignDoctrine 只在"
                    "部署階段生效。"),
            ("語義", "倒戈是開局條件不是戰中事件——"
                    "「內應」存在於開戰前。"),
            ("啟示", "階段限制寫在註解契約——"
        ...
        "code": '// 必須 Deployment 階段呼叫\n// AssignDoctrine 僅部署生效\n// 倒戈是開局條件\n// 不是戰中事件',
        "narration": "第四個設計：只能在部署階段翻。"
                     "叛變必須在開戰前呼叫——"
                     "補編教條只在部署階段生效。"
                     "倒戈是開局條件不是戰中事件："
                     "內應存在於開戰之前。"
    ...
    },
    {
        ...
        "title": "情報是消耗品",
        "sections": [
            ("做法", "HearsayEntry.consumed 位——"
                    "ConsumeVerified 標記已兌換，"
                    "Gather 計數排除已消耗。"),
            ("細節", "檔案仍在只是不計入——"
                    "「檔案在但情報用完了」"
                    "是兩個維度。"),
        ...
        "code": 'e.consumed = true;  // 檔案仍在\n// Gather 排除已消耗\n// 消耗是狀態不是刪除\n// 帳本不擦歷史',
        "narration": "第五個設計：情報是消耗品。"
                     "判詞條目加消耗標記——"
                     "兌換後檔案仍在，"
                     "只是蒐集時不再計入。"
                     "檔案在和情報用完是兩個維度："
    ...
    },
    {
        ...
        "title": "成功才燒情報",
        "sections": [
            ("做法", "ConsumeVerified 只在 "
                    "Subversion Success 呼叫——"
                    "Failed/Backfired 不消耗判詞。"),
            ("語義", "失敗的代價是敵獲首波優勢，"
                    "情報網還在——"
                    "代價和資產分開計。"),
            ("啟示", "消耗掛結果不掛嘗試——"
        ...
        "code": 'if (Success && path==Subversion)\n  dossier.ConsumeVerified(name);\n// 失敗不燒情報\n// 代價=敵先機 資產還在',
        "narration": "第六個設計：成功才燒情報。"
                     "消耗只在顛覆成功時發生——"
                     "失敗和中計不燒判詞，"
                     "代價是敵方先機、情報網還在。"
                     "消耗掛結果不掛嘗試："
    ...
    },
    {
        ...
        "sections": [
            ("湧現", "倒戈數≥敵存活數時照常 "
                    "BeginExecution——首 tick "
                    "CheckOutcome 自然判勝。"),
            ("呈現", "全倒戈才設 peacePathZh——"
                    "戰報走顛覆語域，"
                    "部分倒戈就是帶內應的仗。"),
            ("啟示", "不戰而勝不開特例——"
                    "通用勝利判定接住極端輸入，"
        ...
        "code": 'flipped >= enemyAlive\n  → peacePathZh = 顛覆語域\n// 戰鬥照跑 首tick判勝\n// 特例只在敘事層',
        "narration": "第七個設計：全倒戈是自然勝利。"
                     "倒光敵軍時照常開戰——"
                     "第一個滴答的勝利判定自然接住。"
                     "只有全倒戈才切顛覆語域；"
                     "部分倒戈就是一場帶內應的仗。"
    ...
    },
    {
        ...
        "code": 'recorder.AddRecord(0.0f,\n  "內應倒戈——敵營 N 隊陣前易幟");\n// 戰前事用第零秒釘住',
        "narration": "第八個設計：回放大寫易幟。"
                     "倒戈事件用第零秒時戳記錄——"
                     "戰前發生的事釘在時間軸開頭。"
                     "部署階段的事也要入帳："
                     "回放從第零秒開始完整，"
...
EPISODES = {
    ...
    "ep50": (SLIDES_EP50, "cpp_profiler_50.mp4"),
    "ep51": (SLIDES_EP51, "cpp_lockstep_51.mp4"),
    "ep52": (SLIDES_EP52, "cpp_names_stay_home_52.mp4"),
    "ep53": (SLIDES_EP53, "cpp_first_wave_53.mp4"),
    "ep54": (SLIDES_EP54, "cpp_shorthand_roundtrip_54.mp4"),
    "ep55": (SLIDES_EP55, "cpp_self_trailer_55.mp4"),
    "ep56": (SLIDES_EP56, "cpp_particle_sim_56.mp4"),
    "ep57": (SLIDES_EP57, "cpp_contract_tests_57.mp4"),
    "ep58": (SLIDES_EP58, "cpp_card_validator_58.mp4"),
    "ep59": (SLIDES_EP59, "cpp_defection_59.mp4"),
}
